  ...
  try {
    ...
    if (user) {
      return res
        .status(409)
        .json({
          error: "Account exists with the given email! Log In instead.",
        });
    }
...
const authGoogle = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      res
        .status(200)
        .json({ message: "Oath user" + username + "successfully logged in!" });
    } else {
      user = await Users.create({ username, email, picture });
      req.session.username = username;
